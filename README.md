...
This script can be run in a cronjob or standalone to bypass prompts (not recommended). It relies on a file named "genexus_default.txt", which is the credentials files generated by NYWWS.sh (i.e. {name}_default.txt).
It takes the mandatory argument of the number of days since the last run. To run this every day, '-1' will check for runs that completed one day ago.
